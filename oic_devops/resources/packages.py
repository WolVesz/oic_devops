...
class PackagesResource(BaseResource):
	...
	def get(
		self, package_id: str, params: Optional[Dict[str, Any]] = None
	) -> Dict[str, Any]:
		"""
		...
		for field in required_fields:
			if field not in data:
				raise OICValidationError(
					f'Missing required field for package creation: {field}'
				)

		return super().create(data, params)

	def export(
		self, package_id: str, file_path: str, params: Optional[Dict[str, Any]] = None
	) -> str:
		"""
	...
	def import_package(
		...
		try:
			...
			raise OICAPIError(f'Failed to import package: {e!s}')
	...
	def get_resources(
		...
		response = self.client.get(
			self._get_endpoint(package_id, 'resources'), params=params
		)
		...
		if isinstance(response, list):
			return response
		self.logger.warning(
			f'Unexpected response format from get_resources endpoint: {response.keys() if isinstance(response, dict) else type(response)}'
		)
		return []
	...
	def add_resource(
		...
		for field in required_fields:
			if field not in data:
				raise OICValidationError(
					f'Missing required field for adding resource: {field}'
				)

		return self.client.post(
			self._get_endpoint(package_id, 'resources'), data=data, params=params
		)

	def remove_resource(
		self, package_id: str, resource_id: str, params: Optional[Dict[str, Any]] = None
	) -> Dict[str, Any]:
		"""
