...
def print_status(wf: WorkflowResult, message: str, resource_type:str):
    print("!!!", "=" * 60)
    print(message)
    print("!!!", "-" * 60)
    if wf.resources.get(resource_type):
        ...
        print(f'Resource: {resource_type}, Success:{wf.success}, message:{wf.message}')
...
class PasswordRotationWorkflow(BaseWorkflow):
    ...
    integrations_original_status: Dict[str,Any] | None = None

    RESOURCE_INTEGRATION = "integrations"
    RESOURCE_CONNECTION = "connection"
    ACTION_INTEGRATION_IN_PROGRESS = 'INTEGRATION_IN_PROGRESS'
    ...
    def update_password_basic_authentication(
        ...
        #Validate to continue or not
        continue_true_false = self._prompt_confirm_continue(
            [self.client.config.profile, f"Connections filter: {connection_filter}"],
            "The following environment will be updated.")
        if not continue_true_false:
            result.message = "User canceled update_password_basic_authentication"
            return result

        # 0) Initializations
        timeout_sec = 300 if overrides and not overrides['timeout_sec'] else overrides['timeout_sec']
        poll_interval_sec = 5 if overrides and not overrides['poll_interval_sec'] else overrides['poll_interval_sec']


        result.message = f"Updating passwords. Connection filter: {connection_filter}"
        result.success = True ## till proven guilty
        try:
            ...
            ## Prompt to continue or chicken-out.
            continue_true_false = self._prompt_confirm_continue(connections_dictionary,
                           "The following connections will be Password Rotated. You can still abort after this prompt.")
            if not continue_true_false:
                result.success = False
                result.message = "Aborted at Rotate Passwords. Nothing got updated."
                return result
            ...
            responses_wf = self.enrich_integration_status_with_schedules_snapshot(scheduled_ids, integrations_original_status)
            result.merge(responses_wf)
            if not responses_wf.success:
                return result

            # 3.1) --- Persist  integrations_original_status
            self.integrations_original_status = integrations_original_status
            self.persist_integrations_original_state(integrations_original_status)
            ...
            if not responses_wf.success:
                ...
                result.add_error("Schedulers did not CANCEL on time")
                self.persist_result(result)
                self.print_final_state_comparison(connections_dictionary, integrations_original_status, result)
                return result
            ...
            # Last opportunity to abort
            continue_true_false = self._prompt_confirm_continue(connections_dictionary,
                                                        "LAST opportunity to abort - The following connections will be Password Rotated")
            if not continue_true_false:
                result.success = False
            ...
            if not responses_wf.success:
                ...
                result.add_error("Connections did not UPDATE PASSWORD correctly")
                self.persist_result(result)
                self.print_final_state_comparison(connections_dictionary, integrations_original_status, result)
                return result
            ...
            if not responses_wf.success:
                ...
                result.add_error("Integrations did not activate on time")
                self.persist_result(result)
                self.print_final_state_comparison(connections_dictionary, integrations_original_status, result)
                return result
            ...
            if not responses_wf.success:
                ...
                result.add_error("Schedulers did not re-started")
                self.persist_result(result)
                self.print_final_state_comparison(connections_dictionary, integrations_original_status, result)
                return result
            ...
            if not responses_wf.success:
                ...
                result.add_error("Schedulers did not re-started on time")
                self.persist_result(result)
                self.print_final_state_comparison(connections_dictionary, integrations_original_status, result)
                return result

            self.persist_result(result)
            print("/nTotal errors: ", len(result.errors))
            self.print_final_state_comparison(connections_dictionary, integrations_original_status, result)
            ...
            result.add_error(message=full_error, error=e, resource_id=None)
            self.persist_result(result)
            return result

    def persist_integrations_original_state(self, integrations_original_status: dict[str, dict[str, Any]]):
        try:
            save_dir = self.make_persist_dir()
            ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = os.path.join(save_dir, f"integrations_original_status_{ts}.json")
            with open(filename, "w") as f:
                json.dump(integrations_original_status, f, indent=2, default=str)
            if hasattr(self, "logger"):
                self.logger.info("Saved enriched integrations_original_status snapshot to %s", filename)
            else:
                print(
                    f"[PasswordRotationWorkflow] Saved enriched integrations_original_status snapshot to {filename}")
        except Exception as e:
            if hasattr(self, "logger"):
                self.logger.warning("Failed to save enriched integrations_original_status snapshot: %s", e)
    ...
    def print_final_state_comparison(self, connections_dictionary: dict[str, dict[str, Any]] | None | dict[Any, Any],
        ...
        if not wf.success:
            print(
                "Comparing resulting integrations status cannot be done because of a failure getting the integration status")
        else:
            print(
    ...
    def enrich_integration_status_with_schedules_snapshot(
        self,
        ...
        result = WorkflowResult()
        result.message="snapshot_schedules"
    ...
    def stop_schedulers(
        ...
        result = WorkflowResult()
        result.message = "stop_schedulers"
        overall_success = True

        # Validate to continue or not
        continue_true_false = self._prompt_confirm_continue(
            schedule_integrations,
            "The following schedulers will be STOPPED, if active; otherwise no change")
        if not continue_true_false:
            result.success = False
            result.message = "Aborted at Stop Schedulers. No password has been updated."
            return result
        ...
        for integration_id in schedule_integrations:
            try:
                ...
                result.add_error(resource_id=integration_id, message=msg, error=exc)
    ...
    def start_schedulers(
        ...
        """Starts ACTIVE schedules for the provided integrations."""
        result = WorkflowResult()
        result.message = "start_schedulers"
        overall_success = True
    ...
    def wait_for_integrations_state(
        self,
        integrations_original_status: Iterable[str],
        desired_status: str = "CANCELLED",
        timeout_sec: int = 300,
        poll_interval_sec: int = 5,
    ) -> WorkflowResult:
        result = WorkflowResult()
        result.message = f"wait_for_integrations_state:{desired_status}"  # let's be optimistic
        start = time.time()
        ids = integrations_original_status
        if not ids:
            result.success = True
            result.message = "wait_for_integrations_state:No integrations to wait for"
            return result
        current_status = None
        integration_exception = None
        integration_id_not_matching = "see errors"
        while True:
            all_done = True
            for integration_id in ids:
                try:
                    integration = self.client.integrations.get(integration_id)
                    if integration.get("status") != desired_status:
                        current_status = integration.get("status")
                        self.logger.warning(f'Integration is not at desired status: {integration_id}. current status: {current_status} != desired: {desired_status}')
                        all_done = False
                        integration_id_not_matching = integration_id
                        break
                except OICResourceNotFoundError as nf:
                    self.logger.error(nf)
                    integration_id_not_matching = integration_id
                    integration_exception = nf
                    all_done = False
                    result.add_error(resource_id=integration_id, message=str(nf), error=nf)
                    data = {"action": f"GET Integration - WAIT_{desired_status}", "datetime": datetime.now().isoformat(),
                            "success": False,
                            f"message": f"404 - Not Found: {integration_id}"}
                    result.add_resource(resource_type=self.RESOURCE_INTEGRATION, resource_id=integration_id, data=data)
                except OICError as exc:
                    self.logger.error(exc)
                    integration_exception = exc
                    integration_id_not_matching = integration_id
            ...
            if all_done:
                result.success = True
                for integration_id in ids:
                    result.add_resource(self.RESOURCE_INTEGRATION, integration_id, {"action": f"WAIT_{desired_status}", "success": True, "state": desired_status})
                    result = self.trace_into_details(resource_type=self.RESOURCE_INTEGRATION, workflow_result=result)
                return result

            if time.time() - start >= timeout_sec:
                result.success = False
                result.message = f"wait_for_integrations_state Timeout. Desired state: {desired_status}"
                current_status = current_status if current_status is not None else "UNKNOWN"
                result.add_error(resource_id=integration_id_not_matching,
                                 message=f"Current state: {current_status} desired: {desired_status}. Integration API response: {integration_exception} ")
                message = f"current state: {current_status} desired: {desired_status}"
                data = {"action": "WAIT_{desired_state}", "datetime": datetime.now().isoformat(),"success": False, "message": message}
                result.add_resource(resource_type=self.RESOURCE_INTEGRATION, resource_id=integration_id_not_matching, data=data)
    ...
    def wait_for_schedules_state(
        self,
        schedule_integrations: Iterable[str],
        integrations_original_status: Dict[str, Any],
        ...
        result = WorkflowResult()
        result.message = f"wait_for_schedules_state: {desired_state}"  # let be optimistic
        start = time.time()
        ids = list(schedule_integrations)
        if not ids:
            result.success = True
            result.message = "wait_for_schedules_state: No scheduled integrations to wait for"
            return result
        current_state = None
        ...
        while True:
                ...
                if state == "ACTIVE":
                    try:
                        ...
                        if schedule.get("state") != desired_state:
                            current_state = schedule.get("state")
                            self.logger.warning(f'Integration is not at desired state: {integration_id}. current state: {current_state} != desired: {desired_state}')
                            all_done = False
                            integration_id_not_matching = integration_id
                            break
                    except OICResourceNotFoundError as nf:
                        self.logger.error(nf)
                        integration_id_not_matching = integration_id
                        schedule_exception = nf
                        all_done = False
                        result.add_error(resource_id=integration_id, message=str(nf), error=nf)
                        data = {"action": f"GET Schedule failed on WAIT_SCHEDULE to desired:{desired_state}", "datetime": datetime.now().isoformat(),
                                "success": False,
                                f"message": f"404 - Not Found: {integration_id}"}
                        result.add_resource(resource_type=self.RESOURCE_INTEGRATION, resource_id=integration_id, data=data)
                    except OICError as exc:
                        ...
                        # Conservative: keep waiting if a transient error occurs
                        all_done = False
                        data = {"action": f"GET Schedule failed on WAIT_SCHEDULE to desired:{desired_state}",
                                "datetime": datetime.now().isoformat(),
                                "success": False,
                                f"message": exc}
                        result.add_resource(resource_type=self.RESOURCE_INTEGRATION, resource_id=integration_id,
                                            data=data)
                        break
            ...
            if time.time() - start >= timeout_sec:
                result.success = False
                result.message = f"wait_for_schedules_state Timeout. Desired state: {desired_state}. See errors for details."
                current_state = current_state if current_state is not None else "UNKNOWN"
                result.add_error(resource_id=integration_id_not_matching,
                                 message=f"Current state: {current_state} desired: {desired_state}. Schedule API response: {schedule_exception} ")
                message = f"current state: {current_state} desired: {desired_state}"
                data = {"action": "WAIT_SCHEDULE", "datetime": datetime.now().isoformat(),"success": False, "message": message}
                result.add_resource(resource_type=self.RESOURCE_INTEGRATION, resource_id=integration_id_not_matching, data=data)
    ...
    @staticmethod
    def get_integration_ids_from_connection_dic(connections_dictionary: Dict[str, Dict[str, Any]]) -> List[str]:
    ...
    def get_in_progress_integrations(self, integration_ids: Iterable[str]) -> WorkflowResult:
        ...
        """
        result = WorkflowResult()
        result.message = "get_in_progress_integrations"
        overall_success = True
        all_in_progress = 0
        ...
        for integration_id in integration_ids:
            ...
            try:
                ...
                if total_records_count > 0:
                    ...
                    result.message = f"get_in_progress_integrations - integrations in progress: {all_in_progress}"
                    data = {
                            "action": self.ACTION_INTEGRATION_IN_PROGRESS, "datetime": datetime.now().isoformat(),
                            "message": f"In progress instances: {total_records_count}",
                            "success" : True
                            }
                    result.add_resource(resource_type=self.RESOURCE_INTEGRATION, resource_id=integration_id, data=data)
                ...
                if is_fatal:
                    overall_success = False
                    result.add_error(resource_id=integration_id, message=exc.title, error=exc)
                data = {"action": self.ACTION_INTEGRATION_IN_PROGRESS, "datetime": datetime.now().isoformat(), "success": not is_fatal,
                        "message": exc.title}
                result.add_resource(resource_type=self.RESOURCE_INTEGRATION, resource_id=integration_id, data=data)
            except OICResourceNotFoundError as nf:
                self.logger.warning(nf)
                overall_success = False
                result.add_error(resource_id=integration_id, message=str(nf), error=nf)
                data = {"action": self.ACTION_INTEGRATION_IN_PROGRESS, "datetime": datetime.now().isoformat(), "success": False,
    ...
    def _are_in_progress(self, in_progress_wf_response: WorkflowResult) -> bool:
        in_progress_wf_response.resources.setdefault(self.RESOURCE_INTEGRATION,{})
        resources = in_progress_wf_response.resources[self.RESOURCE_INTEGRATION]
        if resources:
    ...
    def deactivate_integrations(self, integration_ids: Iterable[str]) -> WorkflowResult:
        ...
        result = WorkflowResult()
        result.message = "deactivate_integrations"
        overall_success = True
        ...
        if self._are_in_progress(in_progress_wf_response=in_progress_wf):
            result.message = f"deactivate_integrations: Cannot deactivate because some are running at: {datetime.now().isoformat()}"
            result.merge(in_progress_wf)
            result.success = False
            return result

        continue_true_false = self._prompt_confirm_continue(integration_ids,
                                                   "The following integrations  will be DEACTIVATED")
        if not continue_true_false:
            result.success = False
            result.message = "deactivate_integrations: Aborted. WARNING: Schedules have been stopped"
            return result
    ...
    def activate_integrations(self, integrations_original_status: Dict[str, Any]) -> WorkflowResult:
        """Activate the provided integrations (ignore 412 Precondition)."""
        result = WorkflowResult()
        result.message = "activate_integrations"
        overall_success = True
        originally_activated_ids = self.filter_by_status_is_activated(integrations_original_status)

        for integration_id in originally_activated_ids:
            try:
                response = self.client.integrations.activate(
                    integration_id=integration_id
                )
                # Add activation success  to response
                status = response['status'] if response['status'] else 'unknown'
                data = {"action": self.ACTION_ACTIVATE, "datetime": datetime.now().isoformat(), "success": True, "status": status }
                result.add_resource(resource_type=self.RESOURCE_INTEGRATION, resource_id=integration_id, data=data)
                ...
                if is_fatal:
                    result.add_error(resource_id=integration_id, message=exc.title, error=exc)
                data = {"action": self.ACTION_ACTIVATE, "datetime": datetime.now().isoformat(), "success": not is_fatal, "message": exc.title, "status": "unknown"}
                result.add_resource(resource_type=self.RESOURCE_INTEGRATION, resource_id=integration_id, data=data)
                self.logger.warning("Activating %s has a fatal (%s) exception: %s", integration_id, is_fatal, exc.title)
            except OICResourceNotFoundError as nf:
                overall_success = False
                result.add_error(resource_id=integration_id, message=str(nf), error=nf)
                data = {"action": self.ACTION_ACTIVATE, "datetime": datetime.now().isoformat(), "success": False, "message": "404 - Not Found", "status": "unknown"}
                result.add_resource(resource_type=self.RESOURCE_INTEGRATION, resource_id=integration_id, data=data)
        result = self.trace_into_details(resource_type=self.RESOURCE_INTEGRATION, workflow_result=result)
        result.success = overall_success
        return result

    def filter_by_status_is_activated(self, integrations_original_status: dict[str, Any]) -> list[str]:
        ## Filter originally ACTIVATED
        originally_activated_ids = [
            integration_id
            for integration_id, info in integrations_original_status.items()
            if info.get("status") == "ACTIVATED" or "status" not in info
        ]
        return originally_activated_ids
    ...
    def update_connection_passwords(self, connections_dictionary: Dict[str, Dict[str, Any]], target_usernames: Dict[str, str]) -> WorkflowResult:
        ...
        result = WorkflowResult()
        result.message ='update_connection_passwords'
        overall_success = True
        ...
        for connection_id, info in connections_dictionary.items():
            ...
            try:
                ...
                if configured:
                    data = {"action":"UPDATE_PASSWORD", "success": True, "state":configured}
                    result.add_resource(self.RESOURCE_CONNECTION, connection_id, data)
                    ...
                    data = {"action": "UPDATE_PASSWORD", "success": False, "state": configured, "message":msg}
                    result.add_resource(self.RESOURCE_CONNECTION, connection_id, data=data)
                ...
                data = {"action": "UPDATE_PASSWORD", "success": False, "state": "unknown", "message": msg}
                result.add_resource(self.RESOURCE_CONNECTION, connection_id, data)
            except OICResourceNotFoundError as nf:
                overall_success = False
                result.add_error(resource_id=connection_id, message=str(nf), error=nf)
                data = {"action": "UPDATE_PASSWORD", "datetime": datetime.now().isoformat(), "success": False, "message": "404 - Not Found", "status": "unknown"}
                result.add_resource(resource_type=self.RESOURCE_CONNECTION, resource_id=connection_id, data=data)

        result = self.trace_into_details(resource_type=self.RESOURCE_CONNECTION, workflow_result=result)
        result.success = overall_success
        return result
    ...
    def make_persist_dir(self) -> str:
        ...
        save_dir = os.path.join(save_dir, self.client.config.profile)  # ./output/dev-1
    ...
    @staticmethod
    def trace_into_details(resource_type:str, workflow_result:WorkflowResult)-> WorkflowResult:

        timestamp = datetime.now().isoformat()

        resources = workflow_result.resources.get(resource_type)

        if resources:
            for idx, (integration_id, value) in enumerate(resources.items()):
                key = f"{timestamp}-{idx:07d}"
                workflow_result.details[key] = {integration_id: value}
        return workflow_result


    def _prompt_confirm_continue(self, resource_ids: Iterable[str], message:str) -> bool:
        """
        Prints the ids that will be {message} and prompts for confirmation.
        ...
        print(f"\n{message} in {self.client.config.profile}:")
        for i, resource_id in enumerate(resource_ids, start=1):
            print(f"  {i:>2}.  {resource_id}")

        # Basic y/N prompt
        answer = input("\nProceed ? [y/N]: ").strip().lower()
        return answer in ("y", "yes")

    def restore_integrations_to_original_state(
            self,
            integrations_original_status: Optional[Dict[str, Dict[str, Any]]] =None,
        ...
        integrations_original = integrations_original_status.copy() if integrations_original_status else self.integrations_original_status.copy()
        ...
        integrations_now, result_int = self.get_integrations_snapshot(integration_ids=integrations_original)
        if not result_int.success:
           print_status(wf=result_int, resource_type=self.RESOURCE_INTEGRATION, message="Fail to retrieve integrations snapshot")
           return result_int

        result_sh = self.enrich_integration_status_with_schedules_snapshot(schedule_integration_ids_original, integrations_status=integrations_now)
        if not result_sh.success:
           print_status(wf=result_sh, resource_type=self.RESOURCE_INTEGRATION, message="Fail to retrieve integrations schedulers")
           return result_sh
        ...
        for integration_id, info in integrations_original.items():
            ...
            now_status = integrations_now[integration_id]['status']
            if old_status != now_status:
                different_integration_ids[integration_id] = {"integration_old_status":old_status, "integration_now_status":now_status} # old_status becomes desired
                data = {"old_status":old_status, "now_status":now_status, "datetime": datetime.now().isoformat() }
                result.add_resource(resource_type=self.RESOURCE_INTEGRATION, resource_id=integration_id,data=data)
            old_schedule_state =info.get("SCHEDULE",{}).get("state")
            if old_schedule_state:
                now_schedule_state = integrations_now[integration_id].get("SCHEDULE",{}).get("state")
                if old_schedule_state != now_schedule_state:
                    # different_integration_ids[integration_id] = {"old_schedule_state":old_schedule_state, "now_schedule_state":now_schedule_state}
                    # Append schedule deltas without overwriting existing integration status deltas
                    entry = different_integration_ids.setdefault(integration_id, {})
                    entry.update({"old_schedule_state": old_schedule_state, "now_schedule_state": now_schedule_state})

                    data = {"old_schedule_state": old_schedule_state, "now_schedule_state": now_schedule_state, "datetime": datetime.now().isoformat()}
                    result.add_resource(resource_type=self.RESOURCE_INTEGRATION, resource_id=integration_id, data=data)
        # show Delta
        print("Differences:")
        i = 1
        for integration_id, info in different_integration_ids.items():
            old_status = info.get("integration_old_status")
            if old_status:
                now_status = info['integration_now_status']
                print(f"{i:>4,}.{integration_id} \tINTEGRATION from {now_status} to original status: {old_status}")
                i +=1
            old_schedule_state = info.get("old_schedule_state")
            if old_schedule_state:
                now_schedule_state = info['now_schedule_state']
                print(f"{i:>4,}.{integration_id} \tSCHEDULE from {now_schedule_state} to original: {old_schedule_state}")
                i += 1

        # Proceed?
        continue_true_false = self._prompt_confirm_continue(
            ['looks good?'],
            "The ABOVE integrations will be restored")
        if not continue_true_false:
            result.success = False
        ...
        for integration_id, info in different_integration_ids.items():
            # Active integration, different schedule state
            if info.get("integration_now_status") == "ACTIVATED":
                deactivate_integrations.append(integration_id)
            # Deactive integration
            if info.get("integration_now_status") == "CONFIGURED":
                activate_integrations.append(integration_id)
            # Schedulers
            schedule_old_state = info.get("old_schedule_state")
            if schedule_old_state:  #If it's a Scheduler
                if schedule_old_state == "ACTIVE":
                    start_schedulers.append(integration_id)
                else:
                    stop_schedulers.append(integration_id)


        # Stop schedulers
        if stop_schedulers:
            wf = self.stop_schedulers(stop_schedulers, integrations_now)
            result.merge(wf)
            # Wait until CANCELLED for those we attempted to stop
            wf_wait = self.wait_for_schedules_state(stop_schedulers, integrations_now, desired_state="CANCELLED", timeout_sec=timeout_sec, poll_interval_sec=poll_interval_sec)
            result.merge(wf_wait)

        # Activate Integrations
        if activate_integrations:
            # Build a minimal "original" dict to drive activation of only these ids
            to_activate_dict = {iid: {"status": "ACTIVATED"} for iid in activate_integrations}
            wf_act = self.activate_integrations(to_activate_dict)
            result.merge(wf_act)

            # Wait until ACTIVATED
            wf_wait_act = self.wait_for_integrations_state(activate_integrations, desired_status="ACTIVATED")
            result.merge(wf_wait_act)

        # Start Schedulers
        if start_schedulers:
            wf_start = self.start_schedulers(schedule_integrations=start_schedulers, integrations_original_status=integrations_original)
            result.merge(wf_start)
            # Wait until ACTIVE
            wf_wait_start = self.wait_for_schedules_state(start_schedulers, integrations_original, desired_state="ACTIVE",timeout_sec=timeout_sec, poll_interval_sec=poll_interval_sec)
            result.merge(wf_wait_start)
        print("=="*30)
        print(f"Done restoring to original. Success:{result.success}  Message:{result.message}")
        return result
