...
class ScheduleWorkflows(BaseWorkflow):
	...
	def execute(self, *args, **kwargs) -> WorkflowResult:
		...
		result = WorkflowResult(
			success=False, message=f'Unknown schedule workflow operation: {operation}'
		)
		...
		for int_key, schedule_data in schedule_updates.items():
			...
			if by_identifier:
				try:
					...
					if not matching_integrations:
						self.logger.error(
							f'No integration found with identifier {int_key}'
						)
						error_msg = f'Integration not found with identifier {int_key}'
						failed_updates.append(
							{'identifier': int_key, 'error': error_msg}
						)
					...
					self.logger.error(
						f'Failed to find integration with identifier {int_key}: {e!s}'
					)
					error_msg = f'Failed to find integration: {e!s}'
					failed_updates.append({'identifier': int_key, 'error': error_msg})
			...
			try:
				...
				if integration.get('integrationType') != 'SCHEDULED':
					self.logger.error(
						f'Integration {integration_name} is not a scheduled integration'
					)
					error_msg = (
						f'Integration {integration_name} is not a scheduled integration'
					)
				...
				if 'schedule' not in integration:
					self.logger.error(
						f'Integration {integration_name} has no schedule field'
					)
				...
				self.logger.info(
					f'Updating schedule for integration {integration_name}'
				)
				...
				self.logger.error(
					f'Failed to update schedule for integration {integration_id}: {e!s}'
				)
				error_msg = f'Failed to update schedule: {e!s}'
				failed_updates.append(
					{
						'id': integration_id,
	...
	def export_integration_schedules(
		...
		try:
				...
				for integration_id in integration_ids:
					try:
						...
						if integration.get('integrationType') == 'SCHEDULED':
							...
							self.logger.info(
								f'Integration {integration.get("name", integration_id)} is not a scheduled integration, skipping'
							)
					except OICError as e:
						self.logger.warning(
			...
			for integration in integrations:
				...
				if 'schedule' not in integration:
					self.logger.warning(
						f'Integration {integration_name} has no schedule field, skipping'
					)
					continue
			...
			if format == 'json':
				try:
					...
					self.logger.info(
						f'Exported {len(schedules_data)} schedules to {export_file_path}'
					)
					...
					result.success = False
					result.message = f'Failed to write schedules to JSON file: {e!s}'
				...
				try:
					...
					result.success = False
					result.message = f'Failed to write schedules to CSV file: {e!s}'
				...
				try:
					...
					result.success = False
					result.message = f'Failed to write schedules to YAML file: {e!s}'
			...
			result.success = False
			result.message = f'Failed to export schedules: {e!s}'
	...
	def import_integration_schedules(
		...
		try:
			...
			if file_ext == '.json':
				...
				schedules_data = {}
				with open(import_file_path, newline='') as f:
			...
			for src_key, data in schedules_data.items():
				...
				if not isinstance(data, dict) or 'schedule' not in data:
					self.logger.warning(
						f'Invalid schedule data for key {src_key}, skipping'
					)
					failed_imports.append(
						{'key': src_key, 'error': 'Invalid schedule data format'}
					)
					continue
				...
				try:
					if match_by == 'id':
						...
						try:
							target_integration = self.client.integrations.get(
								integration_id
							)
						except OICError:
							target_integration = None
						...
						matching_integrations = self.client.integrations.list(
							params=params
						)
						target_integration = (
							matching_integrations[0] if matching_integrations else None
						)
						integration_id = (
							target_integration['id'] if target_integration else None
						)
					...
					if not target_integration:
						...
						failed_imports.append({'key': src_key, 'error': error_msg})
					...
					if dry_run:
						self.logger.info(
							f'DRY RUN: Would update schedule for integration {integration_name}'
						)
					...
					error_msg = f'Failed to update schedule: {e!s}'
					self.logger.error(f'Error updating schedule for {src_key}: {e!s}')
					failed_imports.append({'key': src_key, 'error': error_msg})
					result.add_error(
						error_msg,
						e,
						integration_id if 'integration_id' in locals() else None,
					)
			...
			result.success = False
			result.message = f'Failed to import schedules: {e!s}'
	...
	def validate_integration_schedules(
		...
		if validation_rules is None:
			validation_rules = {
				...
				'frequency': None,
				'time_range': {'earliest': '00:00:00', 'latest': '23:59:59'},
		...
		try:
			...
			for integration in integrations:
				...
				if validation_rules.get('time_range') is not None:
					...
					if (
						time_range.get('earliest')
						and schedule_time < time_range['earliest']
					):
						validation_result['valid'] = False
						validation_result['issues'].append(
					...
					if (
						time_range.get('latest')
						and schedule_time > time_range['latest']
					):
						validation_result['valid'] = False
						validation_result['issues'].append(
				...
				if validation_rules.get('allowed_days') is not None:
					if (
						'recurringSchedule' in schedule
						and 'dayOfWeek' in schedule['recurringSchedule']
					):
			...
			result.success = False
			result.message = f'Failed to validate schedules: {e!s}'
		...
		try:
			...
			for integration in integrations:
				...
				if (
					...
					schedule_details['days'] = schedule['recurringSchedule'][
						'dayOfWeek'
					]
				else:
				...
				result.add_resource(
					'integration',
					integration_id,
					{'name': integration_name, 'schedule': schedule},
				)
			...
			result.success = False
			result.message = f'Failed to list schedules: {e!s}'
