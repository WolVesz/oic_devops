...
class IntegrationWorkflows(BaseWorkflow):
	...
	def bulk_activate_integrations(
		...
		if integration_ids:
			...
			self.logger.info(
				f'Using provided list of {len(integration_ids)} integration IDs'
			)
		...
		for integration in integrations_to_activate:
			...
			if current_status == 'ACTIVATED':
				self.logger.info(
					f'Integration {integration_name} is already activated, skipping'
				)
			...
			try:
				...
				if verify_activation:
					...
					try:
						...
						if new_status != 'ACTIVATED':
							self.logger.warning(
								f'Integration {integration_name} not fully activated, status: {new_status}'
							)
							activation_verified = False
					except OICError as e:
						self.logger.error(
							f'Failed to verify activation of integration {integration_name}: {e!s}'
						)
						activation_verified = False
				...
				if activation_verified:
					...
					result.add_error(
						f'Activation verification failed for {integration_name}',
				...
				if (
					sequential and integration != integrations_to_activate[-1]
				):  # Not the last one
				...
				failed_activations.append(
					{'id': integration_id, 'name': integration_name, 'error': str(e)}
				)
				result.add_error(
					f'Failed to activate integration {integration_name}',
					e,
					integration_id,
				)
				result.add_resource(
		...
		if failed_activations:
			...
			result.message = (
				f'Successfully activated {len(successful_activations)} integrations'
			)
	...
	def bulk_deactivate_integrations(
		...
		for integration in integrations_to_deactivate:
			...
			if current_status != 'ACTIVATED':
				self.logger.info(
					f'Integration {integration_name} is not activated (status: {current_status}), skipping'
				)
			...
			try:
				...
				if verify_deactivation:
					...
					try:
						...
						if new_status == 'ACTIVATED':
							self.logger.warning(
								f'Integration {integration_name} still activated, deactivation failed'
							)
							deactivation_verified = False
					except OICError as e:
						self.logger.error(
							f'Failed to verify deactivation of integration {integration_name}: {e!s}'
						)
						deactivation_verified = False
				...
				if deactivation_verified:
					...
					result.add_error(
						f'Deactivation verification failed for {integration_name}',
						resource_id=integration_id,
					)
					result.add_resource(
						'integration',
				...
				if (
					sequential and integration != integrations_to_deactivate[-1]
				):  # Not the last one
				...
				result.add_error(
					f'Failed to deactivate integration {integration_name}',
					e,
					integration_id,
				)
				result.add_resource(
		...
		if failed_deactivations:
			...
			result.message = (
				f'Successfully deactivated {len(successful_deactivations)} integrations'
			)
	...
	def manage_integration_schedules(
			...
			for integration_id in integration_ids:
				try:
					...
					if integration_type != 'SCHEDULED':
						self.logger.warning(
							f'Integration {integration.get("name", integration_id)} is not scheduled (type: {integration_type}), skipping'
						)
						continue
				...
				for integration in integrations:
					...
					if integration_id:
						...
						try:
							...
							self.logger.error(
								f'Failed to get details for integration {integration_id}: {e!s}'
							)
							if not continue_on_error:
								result.add_error(
		...
		self.logger.info(
			f'Managing schedules for {len(integrations_to_manage)} integrations'
		)
		...
		for integration in integrations_to_manage:
			...
			try:
				...
				if 'schedule' not in integration_detail:
					self.logger.warning(
						f'Integration {integration_name} does not have a schedule field'
					)
					...
					result.add_error(
						f'No schedule field in integration {integration_name}',
						resource_id=integration_id,
					)
					result.add_resource(
						'integration',
				...
				self.logger.info(
					f'Updating integration {integration_name} to {action_description} schedule'
				)
				update_result = self.client.integrations.update(
					integration_id, integration_detail
				)
				...
				self.logger.error(
					f'Failed to {action} schedule for integration {integration_name}: {e!s}'
				)
				failed_operations.append(
					{'id': integration_id, 'name': integration_name, 'error': str(e)}
				)
		...
		if 'references' in integration and isinstance(integration['references'], list):
			for ref in integration['references']:
				...
				if include_connections and ref_type == 'CONNECTION':
					connections.append({'id': ref_id, 'name': ref_name})
					result.add_resource('connection', ref_id, {'name': ref_name})

				elif include_lookups and ref_type == 'LOOKUP':
					lookups.append({'id': ref_id, 'name': ref_name})
					result.add_resource('lookup', ref_id, {'name': ref_name})

				elif include_libraries and ref_type == 'LIBRARY':
					libraries.append({'id': ref_id, 'name': ref_name})
					result.add_resource('library', ref_id, {'name': ref_name})
					...
					for item in integration[section]:
						...
						if conn_id:
							...
							if not any(conn['id'] == conn_id for conn in connections):
								connections.append({'id': conn_id, 'name': conn_name})
								result.add_resource(
									'connection', conn_id, {'name': conn_name}
								)
	...
	def restart_integration(
		self, integration_id: str, verify_restart: bool = True, wait_time: int = 10
	) -> WorkflowResult:
		"""
		...
		try:
			...
			result.add_resource(
				'integration',
				integration_id,
				{'name': integration_name, 'initial_status': current_status},
			)
			result.details['integration_name'] = integration_name
		...
		if current_status == 'ACTIVATED':
			try:
				...
				if verify_restart:
					try:
						...
						if integration_status.get('status') == 'ACTIVATED':
							self.logger.warning(
								f'Integration {integration_name} still activated after deactivation attempt'
							)
							result.add_error(
								'Deactivation verification failed',
								resource_id=integration_id,
							)
							result.success = False
							result.message = (
						...
						self.logger.error(f'Failed to verify deactivation: {e!s}')
						result.add_error(
							'Failed to verify deactivation', e, integration_id
						)
						result.success = False
				...
				result.add_error('Failed to deactivate integration', e, integration_id)
				result.success = False
		...
		try:
			...
			if verify_restart:
				try:
					...
					result.add_resource(
						'integration', integration_id, {'final_status': final_status}
					)

					if final_status != 'ACTIVATED':
						self.logger.warning(
							f'Integration {integration_name} not fully activated, status: {final_status}'
						)
						result.add_error(
							'Activation verification failed', resource_id=integration_id
						)
						result.success = False
					...
					self.logger.error(f'Failed to verify activation: {e!s}')
					result.add_error('Failed to verify activation', e, integration_id)
					result.success = False
					result.message = (
						f'Failed to verify activation of integration {integration_name}'
					)
					return result
			...
			result.add_error('Failed to activate integration', e, integration_id)
			result.success = False
	...
	def trace_integration_instances(
		...
		try:
			if instance_id:
				...
				self.logger.info(
					f'Getting integration instances with filters: {params}'
				)
				...
				if len(instances) > max_instances:
					self.logger.info(
						f'Limiting to {max_instances} of {len(instances)} instances'
					)
			...
			self.logger.error(f'Failed to get instances: {e!s}')
		...
		for instance in instances:
			...
			if include_activities and instance_id:
				try:
					...
					activities = self.client.monitoring.get_instance_activities(
						instance_id
					)

					# Add activities to the instance resource
					instance_resource = result.resources.get('instance', {}).get(
						instance_id, {}
					)
					...
					for activity in activities:
						...
						if include_payloads and activity_id:
							...
							try:
								request_payload = (
									self.client.monitoring.get_instance_payload(
								...
								self.logger.warning(
									f'Failed to get request payload for activity {activity_id}: {e!s}'
								)
							...
							try:
								...
								self.logger.warning(
									f'Failed to get response payload for activity {activity_id}: {e!s}'
								)
					...
					self.logger.warning(
						f'Failed to get activities for instance {instance_id}: {e!s}'
					)
