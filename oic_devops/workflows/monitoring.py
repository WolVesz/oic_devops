...
class MonitoringWorkflows(BaseWorkflow):
    ...
    def execute(self, *args, **kwargs) -> WorkflowResult:
        ...
        result = WorkflowResult(
            success=False, message=f'Unknown monitoring workflow operation: {operation}'
        )
    ...
    def perform_health_check(
        ...
        try:
            ...
            self.logger.error(f'Failed to check instance health: {e!s}')
        ...
        if check_integrations:
            try:
                ...
                self.logger.error(f'Failed to check integration health: {e!s}')
        ...
        if check_connections:
            try:
                ...
                for connection in connections:
                    ...
                    if test_connections and connection_id:
                        try:
                            ...
                            self.logger.warning(
                                f'Failed to test connection {connection_name}: {e!s}'
                            )
                            connection_health = 'degraded'
                            problematic_connections.append(
                                {
                ...
                self.logger.error(f'Failed to check connection health: {e!s}')
        ...
        if overall_health == 'healthy':
            ...
            result.message = (
                f'OIC health check found issues with: {", ".join(health_parts)}'
            )
    ...
    def analyze_errors(
        ...
        result.message = (
            f'Analyzing errors{integration_str}{error_str} {time_range_str}'
        )
        ...
        try:
            ...
            monitoring_errors = self.client.monitoring.get_errors(
                start_time=api_start_time, end_time=api_end_time
            )
            ...
            if generate_report:
                ...
                try:
                    ...
                    self.logger.error(f'Failed to generate error report: {e!s}')
            ...
            self.logger.error(f'Failed to analyze errors: {e!s}')
            result.add_error('Failed to analyze errors', e)
            result.success = False
            result.message = f'Failed to analyze errors: {e!s}'
    ...
    def collect_performance_metrics(
        ...
        result.message = (
            f'Collecting performance metrics{integration_str} {time_range_str}'
        )
        ...
        try:
            ...
            # Process metrics
            perf_metrics = {'counts': {}, 'durations': {}, 'errors': {}, 'summary': {}}

            # Process integration stats for counts and errors
            if (
                'status' in integration_stats
                and integration_stats['status'] == 'SUCCESS'
            ):
                ...
                if 'counts' in metrics and 'counts' in stats_data:
                    ...
                    count_summary = {'total': 0, 'by_status': {}, 'by_integration': {}}
                    ...
                    for period, period_data in stats_data['counts'].items():
                        for status, status_count in period_data.items():
                            ...
                            if 'integrationId' in period_data:
                                ...
                                integration_name = period_data.get(
                                    'integrationName', 'Unknown'
                                )

                                if (
                                    integration_id
                                    not in count_summary['by_integration']
                                ):
                                ...
                                int_summary = count_summary['by_integration'][
                                    integration_id
                                ]
                ...
                if 'errors' in metrics and 'errors' in stats_data:
                    ...
                    error_summary = {'total': 0, 'by_type': {}, 'by_integration': {}}
                    ...
                    for period, period_data in stats_data['errors'].items():
                        for error_type, error_count in period_data.items():
                            ...
                            if 'integrationId' in period_data:
                                ...
                                if (
                                    integration_id
                                    not in error_summary['by_integration']
                                ):
                                ...
                                int_summary = error_summary['by_integration'][
                                    integration_id
                                ]
            ...
            if 'durations' in metrics:
                ...
                for instance in instances:
                    ...
                    try:
                        ...
                        if interval == 'hour':
                            ...
                            week_start = start_dt - datetime.timedelta(
                ...
                for integration_id, int_data in durations_by_integration.items():
                    if int_data['count'] > 0:
                        ...
                        int_data['average_seconds'] = (
                            int_data['total_seconds'] / int_data['count']
                        )
                        ...
                        duration_summary['total_duration_seconds'] += int_data[
                            'total_seconds'
                        ]
                ...
                for period_key, period_data in durations_by_period.items():
                    if period_data['count'] > 0:
                        ...
                        period_data['average_seconds'] = (
                            period_data['total_seconds'] / period_data['count']
                        )
                        ...
                        for integration_id, int_data in period_data[
                            'by_integration'
                        ].items():
                            if int_data['count'] > 0:
                                int_data['average_seconds'] = (
                ...
                if all_durations:
                    duration_summary['average_duration_seconds'] = sum(
                        all_durations
                    ) / len(all_durations)
            ...
            if generate_report:
                ...
                try:
                    ...
                    if (
                        ...
                        and perf_metrics['durations']['by_period']
                    ):
                        ...
                        with open(csv_file, 'w', newline='') as f:
                            ...
                            # Write data
                            for period, period_data in sorted(
                                perf_metrics['durations']['by_period'].items()
                            ):
                                if period_data['count'] > 0:
                                    # Write period total
                                    writer.writerow(
                                    ...
                                    for int_id, int_data in sorted(
                                        period_data['by_integration'].items(),
                                        key=lambda x: x[1]['count'],
                                        reverse=True,
                                    ):
                                        if int_data['count'] > 0:
                    ...
                    self.logger.error(f'Failed to generate performance report: {e!s}')
            ...
            if metrics_summary:
                if integration_id:
                    result.message = (
                        f'Collected metrics for integration {integration_id} {time_range_str}: '
                        + ', '.join(metrics_summary)
                    )
                else:
                    result.message = (
                        f'Collected metrics {time_range_str}: '
                        + ', '.join(metrics_summary)
                    )

                if generate_report and 'report_file' in result.details:
                    result.message += (
                        f'. Report generated: {result.details["report_file"]}'
                    )
            else:
                result.message = f'No metrics data available {time_range_str}'

        except OICError as e:
            self.logger.error(f'Failed to collect performance metrics: {e!s}')
            result.add_error('Failed to collect performance metrics', e)
            result.success = False
            result.message = f'Failed to collect performance metrics: {e!s}'
        ...
        try:
            ...
            for integration_id, integration_data in instances_by_integration.items():
                ...
                for i in range(0, len(integration_instances), batch_size):
                    ...
                    self.logger.info(
                        f'Purging batch of {batch_size_actual} instances for integration {integration_name}'
                    )

                    try:
                        # Prepare purge data
                        purge_data = {'integrationId': integration_id}
                        ...
                        # Perform purge
                        purge_result = self.client.monitoring.purge_instances(
                            purge_data
                        )
                        ...
                        self.logger.info(
                            f'Purged {purged_count} instances for integration {integration_name}'
                        )
                        ...
                        result.add_resource(
                            'integration',
                            integration_id,
                            {'name': integration_name, 'purged_count': purged_count},
                        )

                    except OICError as e:
                        self.logger.error(
                            f'Failed to purge instances for integration {integration_name}: {e!s}'
                        )
                        result.add_error(
                            f'Failed to purge instances for integration {integration_name}',
                            e,
                            integration_id,
                        )
                        result.success = False
            ...
            self.logger.error(f'Failed to purge instances: {e!s}')
            result.add_error('Failed to purge instances', e)
            result.success = False
            result.message = f'Failed to purge instances: {e!s}'
    ...
    def generate_monitoring_report(
        ...
        if report_dir and not os.path.exists(report_dir):
            try:
                ...
                self.logger.error(
                    f'Failed to create report directory {report_dir}: {e!s}'
                )
                result.add_error('Failed to create report directory', e)
                result.success = False
                result.message = f'Failed to generate report: {e!s}'
                return result
        ...
        try:
            if report_type == 'errors' or report_type == 'full':
                ...
                error_workflow = self.analyze_errors(
                    start_time=start_time, end_time=end_time, generate_report=False
                )

                if error_workflow.success:
                    report_data['errors'] = error_workflow.details.get(
                        'error_analysis', {}
                    )
                    report_data['error_summary'] = error_workflow.details.get(
                        'error_summary', {}
                    )
                else:
            ...
            if report_type == 'performance' or report_type == 'full':
                ...
                if perf_workflow.success:
                    report_data['performance'] = perf_workflow.details.get(
                        'performance_metrics', {}
                    )
                else:
            ...
            if report_type == 'usage' or report_type == 'full':
                ...
                try:
                    ...
                    active_integrations = [
                        i for i in integrations if i.get('status') == 'ACTIVATED'
                    ]
                    ...
                    self.logger.warning(
                        f'Failed to collect usage data for report: {e!s}'
                    )
            ...
            if report_type == 'health' or report_type == 'full':
                ...
                if health_workflow.success:
                    report_data['health'] = {
                        'overall_health': health_workflow.details.get(
                            'overall_health', 'unknown'
                        ),
                        'instance_health': health_workflow.details.get(
                            'instance_health', {}
                        ),
                        'integration_health': health_workflow.details.get(
                            'integration_health', {}
                        ),
                        'connection_health': health_workflow.details.get(
                            'connection_health', {}
                        ),
                    }
                else:
            ...
            if output_format == 'json':
                try:
                    ...
                    self.logger.error(f'Failed to write JSON report: {e!s}')
                ...
                try:
                    ...
                    if 'error_summary' in report_data:
                        ...
                        with open(errors_file, 'w', newline='') as f:
                            ...
                            if (
                                'top_errors_by_integration'
                                in report_data['error_summary']
                            ):
                                writer.writerow(
                                    ['Integration', 'Error Count', 'Top Error Types']
                                )
                                for integration in report_data['error_summary'][
                                    'top_errors_by_integration'
                                ]:
                                    writer.writerow(
                                        [
                                            ...
                                            ', '.join(
                                                [
                                                    f'{err_type}: {err_count}'
                                                    for err_type, err_count in integration[
                                                        'top_error_types'
                                                    ]
                                                ]
                                            ),
                                ...
                                writer.writerow(
                                    ['Error Type', 'Error Count', 'Top Integrations']
                                )
                                for error in report_data['error_summary'][
                                    'top_error_types'
                                ]:
                                    top_integrations = [
                                        f'{int_id}: {int_data["count"]}'
                                        for int_id, int_data in error[
                                            'top_integrations'
                                        ][:3]
                                    ]
                                    writer.writerow(
                    ...
                    if (
                        'performance' in report_data
                        and 'summary' in report_data['performance']
                    ):
                        ...
                        if 'durations' in perf_summary:
                            ...
                            with open(durations_file, 'w', newline='') as f:
                                ...
                                for int_id, int_data in perf_summary['durations'][
                                    'by_integration'
                                ].items():
                                    writer.writerow(
                                        [
                    ...
                    if 'health' in report_data:
                        ...
                        with open(health_file, 'w', newline='') as f:
                            ...
                            writer.writerow(
                                ['Overall', report_data['health']['overall_health']]
                            )
                            writer.writerow(
                                [
                                    'Instance',
                                    report_data['health']['instance_health'].get(
                                        'status', 'unknown'
                                    ),
                                ]
                            )
                            writer.writerow(
                                [
                                    'Integrations',
                                    report_data['health']['integration_health'].get(
                                        'status', 'unknown'
                                    ),
                                ]
                            )
                            writer.writerow(
                                [
                                    'Connections',
                                    report_data['health']['connection_health'].get(
                                        'status', 'unknown'
                                    ),
                                ]
                            )
                    ...
                    if 'usage' in report_data:
                        ...
                        with open(usage_file, 'w', newline='') as f:
                            ...
                            writer.writerow(
                                [
                                    'Total Integrations',
                                    report_data['usage']['integration_counts']['total'],
                                ]
                            )
                            writer.writerow(
                                [
                                    'Active Integrations',
                                    report_data['usage']['integration_counts'][
                                        'active'
                                    ],
                                ]
                            )
                    ...
                    self.logger.error(f'Failed to write CSV report: {e!s}')
                ...
                try:
                    ...
                    if 'health' in report_data:
                        ...
                        if (
                            'integration_health' in health_data
                            and 'problematic' in health_data['integration_health']
                            and health_data['integration_health']['problematic']
                        ):
                            html_content += """
                            ...
                            for integration in health_data['integration_health'][
                                'problematic'
                            ]:
                                html_content += f"""
                                <tr>
                        ...
                        if (
                            'connection_health' in health_data
                            and 'problematic' in health_data['connection_health']
                            and health_data['connection_health']['problematic']
                        ):
                            html_content += """
                            ...
                            for connection in health_data['connection_health'][
                                'problematic'
                            ]:
                                html_content += f"""
                                <tr>
                    ...
                    if 'error_summary' in report_data:
                        ...
                        if 'top_errors_by_integration' in report_data['error_summary']:
                            ...
                            for error in report_data['error_summary'][
                                ...
                                top_integrations = [
                                    f'{int_data["name"]}: {int_data["count"]}'
                                    for int_id, int_data in error['top_integrations'][
                                        :3
                                    ]
                                ]
                                html_content += f"""
                    ...
                    if (
                        ...
                        if 'durations' in perf_summary:
                            ...
                            for int_id, int_data in sorted(
                                durations['by_integration'].items(),
                                key=lambda x: x[1]['average_seconds'],
                                reverse=True,
                            ):
                                html_content += f"""
                                <tr>
                    ...
                    self.logger.error(f'Failed to write HTML report: {e!s}')
            ...
            if result.success:
                if output_format == 'csv' and 'report_files' in result.details:
                    ...
                    result.message = (
                        f'Successfully generated {report_type} report: {report_file}'
                    )
            else:
                result.message = f'Failed to generate complete {report_type} report'

        except Exception as e:
            self.logger.error(f'Failed to generate report: {e!s}')
            result.add_error('Failed to generate report', e)
            result.success = False
            result.message = f'Failed to generate {report_type} report: {e!s}'

        return result


    def download_activity_streams_and_payloads(
        self,
        integration_code: str,
        output_dir: Union[str, Path],
        *,
        timewindow: str = "RETENTIONPERIOD",
        group_by: Optional[str] = "integration",
        timezone: str = "America/Denver",
        limit_instances: Optional[int] = None,
        write_intermediate_index: bool = True,
    ) -> Dict[str, Any]:
        """
        For a given integration_code:
          1) Get integration instances (paginated) via MonitoringResource.get_integration_instances_all(...)
          2) For each instanceId:
             - Download Activity Stream JSON (writes a file in the instance folder)
             - Discover payload links (links[rel=='payload'])
             - Extract tracking-data (encoded) from the href
             - Download each payload XML (writes files under <instance>/payloads)

        Args:
            integration_code: The OIC integration code (e.g., 'PSSWRD_SUBSCRIBER_B').
            output_dir: Base directory where all outputs will be written.
            timewindow: Time window for instances (e.g., '1h', '6h', '1d', 'RETENTIONPERIOD').
            group_by: Grouping for the instances API (commonly 'integration').
            timezone: Time zone used when calling Activity Streams APIs.
            limit_instances: Optional cap on how many instances to process.
            write_intermediate_index: If True, writes the raw instances index JSON.

        Returns:
            Summary dict with counts and per-instance details.
        """

        # ----- helpers kept local so this method is self-contained -----
        ...
        def _extract_tracking_from_href(href: str) -> Tuple[str, Dict[str, str]]:
            """
            Given a payload link HREF like:
              .../activityStreamDetails/tracking-data%2Fdebug%2FABCDEF?integrationInstance=prod-xxx
            return:
              ("tracking-data%2Fdebug%2FABCDEF", {"integrationInstance": "prod-xxx"})
            NOTE: We intentionally return the *encoded* tracking-data exactly as in the URL.
            """
            parts = urlsplit(href)
            path = parts.path
            key = "/activityStreamDetails/"
            if key not in path:
                raise ValueError(f"Unexpected payload href path (no '{key}'): {href}")
            tracking_data = path.split(key, 1)[1]
            q = dict(parse_qsl(parts.query)) if parts.query else {}
            return tracking_data, q

        def _extract_instance_ids(integration_items: Iterable[Dict[str, Any]]) -> List[Tuple[str, Optional[str]]]:
            """
            integration_items can be:
              - a list where each element has an 'instances' list with 'instanceId', or
              - a list of dicts that directly have 'instanceId' or 'id'.
            """
            ids: List[Tuple[str, Optional[str]]] = []
            for item in integration_items or []:
                if not isinstance(item, dict):
                    continue
                # Case 1: grouped payload with 'instances' array
                inst_list = item.get("instances")
                if isinstance(inst_list, list):
                    for inst in inst_list:
                        inst_id = (inst or {}).get("instanceId") or (inst or {}).get("id") or (inst or {}).get("instance_id")
                        created_date = (inst or {}).get("creationDate", datetime.datetime.now().strftime('%Y%m%d-%H%M%S'))
                        if inst_id:
                            ids.append((inst_id, created_date))
                    continue

            return ids

        # ----- resolve monitoring resource & logger -----
        mon = self.client.monitoring
        logger = getattr(self, "logger", None)

        # ----- layout roots -----
        base_dir = Path(output_dir).expanduser().resolve() / _safe_filename(integration_code)
        base_dir.mkdir(parents=True, exist_ok=True)

        # ----- 1) get integration instances index -----
        instances_index: List[Dict[str, Any]] = mon.get_integration_instances_all(
            timewindow=timewindow,
            integration_code=integration_code,
            group_by=group_by,
        )

        # Optional: write raw index for debugging/auditing
        if write_intermediate_index:
            idx_path = base_dir / f"{_safe_filename(integration_code)}_get_integration_instances_all.json"
            try:
                with idx_path.open("w", encoding="utf-8") as f:
                    json.dump(instances_index, f, ensure_ascii=False, indent=2)
                if logger:
                    logger.info(f"Wrote instances index: {idx_path}")
            except Exception as e:
                if logger:
                    logger.error(f"Failed writing instances index to {idx_path}: {e}")

        # Prepare the list of instance IDs
        instances = _extract_instance_ids(instances_index)
        if limit_instances is not None:
            instances = instances[: int(limit_instances)]

        total_payloads = 0
        per_instance = []

        # ----- 2) iterate instances -----
        for  instance_id, creation_date in instances:
            inst_dir = base_dir / _safe_filename(f"{creation_date}")
            inst_dir.mkdir(parents=True, exist_ok=True)

            # 2a) Activity Stream JSON (file is written by the MonitoringResource method)
            try:
                details_items = mon.get_instance_activity_stream_details(
                    activity_stream_details_instance_id=instance_id,
                    params={"timezone": timezone},
                    file_dir=inst_dir.as_posix()
                )
            except Exception as e:
                if logger:
                    logger.error(f"[{instance_id}] Failed to fetch activity stream details: {e}")
                # Skip to next instance
                continue

            # 2b) Collect payload links from items
            payload_links: List[Tuple[str, Dict[str, str]]] = []
            for item in details_items or []:
                for link in (item or {}).get("links", []):
                    if (link or {}).get("rel") == "payload" and (link or {}).get("href"):
                        try:
                            tracking_data, qs = _extract_tracking_from_href(link["href"])
                            payload_links.append((tracking_data, qs))
                            break
                        except Exception as e:
                            if logger:
                                logger.warning(f"[{instance_id}] Skipping malformed payload link '{link.get('href')}': {e}")

            # 2c) De-dupe per instance
            seen_tracking: Set[str] = set()
            unique_payloads: List[Tuple[str, Dict[str, str]]] = []
            for td, qs in payload_links:
                if td not in seen_tracking:
                    unique_payloads.append((td, qs))
                    seen_tracking.add(td)

            # 2d) Download payloads
            payload_dir = inst_dir


            downloaded_for_instance = 0
            for pidx, (tracking_data, qs) in enumerate(unique_payloads, start=1):

                # Preserve encoded tracking_data; add timezone and integrationInstance if present
                call_params = {"timezone": timezone}
                if "integrationInstance" in qs:
                    call_params["integrationInstance"] = qs["integrationInstance"]

                try:
                    mon.get_instance_activity_stream_details_payload(
                        activity_stream_details_instance_id=instance_id,
                        tracking_data=tracking_data,     # keep encoded as found in href
                        params=call_params,
                        file_dir=payload_dir.as_posix(),              # writes XML; returns XML text as str
                    )
                    downloaded_for_instance += 1
                except Exception as e:
                    if logger:
                        logger.error(f"[{instance_id}] Failed to download payload '{tracking_data}' → {out_path}: {e}")

            total_payloads += downloaded_for_instance
            per_instance.append(
                {
                    "instanceId": instance_id,
                    "activityStreamDir": str(inst_dir),
                    "payloadsDownloaded": downloaded_for_instance,
                    "payloadDir": str(payload_dir),
                }
            )

            if logger:
                logger.info(f"[{instance_id}/{len(instances)}] instanceId={instance_id} → payloads: {downloaded_for_instance}")
        ...
        summary = {
            ...
            "payloadsDownloaded": total_payloads,
